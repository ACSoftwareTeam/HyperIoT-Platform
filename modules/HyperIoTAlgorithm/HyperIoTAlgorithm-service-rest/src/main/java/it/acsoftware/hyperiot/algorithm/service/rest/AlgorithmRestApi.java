...
public class AlgorithmRestApi extends HyperIoTBaseEntityRestApi<Algorithm>  {
	...
	@PUT
	@Path("/{algorithmId}/jar/{mainClassname}")
	@Produces(MediaType.APPLICATION_JSON)
	@LoggedIn
	@ApiOperation(value = "/hyperiot/algorithms/{algorithmId}/jar/{mainClassName}", notes = "Service for updating jar of algorithm",
	...
	@ApiResponses(value = { @ApiResponse(code = 200, message = "Successful operation"),
			...
			@ApiResponse(code = 500, message = "Internal error") })
	@JsonView(HyperIoTJSONView.Public.class)
	public Response updateJar(
			@ApiParam(value = "Algorithm ID which updates jar for", required = true) @PathParam("algorithmId") long algorithmId,
			@ApiParam(value = "Classname containing main method", required = true) @PathParam("mainClassname") String mainClassname,
			@ApiParam(value = "Jar file ", required = true) File jar) {
		getLog().debug( "In Rest Service PUT /hyperiot/algorithms/{}/jar" , algorithmId);
		try {
			Algorithm response = entityService.updateJar(getHyperIoTContext(), algorithmId, mainClassname, jar);
			return Response.ok(response).build();
		} catch (Throwable t) {
			return this.handleException(t);
		}
